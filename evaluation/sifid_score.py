...
from .sifid_inception import InceptionV3
import torchvision
import numpy
import scipy
import pickle
...
def calculate_activation_statistics(files, model, batch_size=1,
    ...
    # if len(act.shape) > 2:
    #     mu = np.array([np.mean(a, axis=0) for a in act])
    #     sigma = np.array([np.cov(a, rowvar=False) for a in act])
    # else:
...
def calculate_sifid_given_paths(path1, path2, batch_size=1, cuda=True, dims=64, suffix='jpg', pair=False):
    """Calculates the SIFID of two paths"""

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims] # =0

    model = InceptionV3([block_idx])
    if cuda:
        model.cuda()

    path1 = pathlib.Path(path1)  # path1应为文件夹名称
    files1 = sorted(list(path1.glob('*.%s' %suffix)))  # 获取路径下所有匹配的文件，即所有结尾为.jpg的文件
    path2 = pathlib.Path(path2)
    files2 = sorted(list(path2.glob('*.%s' %suffix)))

    sifid_values = []
    if pair:
        # for ii in range(0, len(files2), batch_size):
        #     m1, s1 = calculate_activation_statistics(files1[ii:ii + batch_size], model, batch_size, dims, cuda)
        #     m2, s2 = calculate_activation_statistics(files2[ii:ii + batch_size], model, batch_size, dims, cuda)
        #     for m11, s11, m22, s22 in zip(m1, s1, m2, s2):
        #         sifid_values.append(calculate_frechet_distance(m11, s11, m22, s22))
        ...
        for f in range(len(files1)):
            fid_values = []
            m1, s1 = calculate_activation_statistics([files1[f]], model, batch_size, dims, cuda)
            for ff in range(len(files2)):
                # m1, s1 = calculate_activation_statistics([files1[i]], model, batch_size, dims, cuda)
                m2, s2 = calculate_activation_statistics([files2[ff]], model, batch_size, dims, cuda)
                fid_values.append(calculate_frechet_distance(m1, s1, m2, s2))
            fid_values = np.asarray(fid_values, dtype=np.float32)
            sifid_values.append(fid_values.mean())
...
if __name__ == '__main__':
    ...
    sifid_values = calculate_sifid_given_paths(path1, path2, 1, True, 64, suffix)
    sifid_values = np.asarray(sifid_values,dtype=np.float32)
    # numpy.save('SIFID', sifid_values)
    print('SIFID: ', sifid_values)
    fp = open(path2.split('/')[-3] + '/' + path2.split('/')[-2] + '_SIFID.txt', 'a+')
    fp.write(path1.split('/')[-1] + ":" + str(sifid_values.mean()) + '\n')
    fp.close()
