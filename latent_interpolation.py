...
if __name__ == '__main__':
    ...
    z1 = fixed_z[3].to(device).unsqueeze(0)
    z2 = fixed_z[10].to(device).unsqueeze(0)
    print(z1.shape, z2.shape)
    num_steps = 12

    # # Create a tensor of interpolation factors from 0 to 1
    # steps = torch.linspace(0, 1, num_steps).unsqueeze(1).to(device)  # Shape [12, 1] to broadcast properly
    #
    # # Interpolate between z1 and z2
    # interpolated_codes = (1 - steps) * z1 + steps * z2  # Broadcast multiplication
    ...
    non_linear_steps = non_linear_steps.unsqueeze(1).to(device)  # Shape [12, 1] for broadcasting

    # Perform the interpolation
    interpolated_codes = (1 - non_linear_steps) * z1 + non_linear_steps * z2
    ...
    if args.mode == 'eval_FIDnISnLPIPS':
        ...
        generate_imgs_4FIDnISnLPIPS(args, g_target, latent_code=interpolated_codes)
